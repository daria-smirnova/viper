...
def codex_helper(extended_prompt):
    ...
    assert 0 <= config.codex.temperature <= 1, "Temperature must be between 0 and 1."
    ...
    if config.codex.model in ("gpt-4", "gpt-3.5-turbo"):
        ...
        # Creating responses using the ChatCompletion API
        ...
        print("Responses is ---------------------------------------------" + str(responses))
        
        # Extracting content from responses using dot notation
        resp = [
        ...
        # Handling response extraction based on the input type
...
class CodexModel(BaseModel):
    ...
    def forward(self, prompt, input_type='image', prompt_file=None, base_prompt=None, extra_context=None):
        ...
        # Load base prompt from a file if specified
        ...
        if base_prompt is None:
            raise ValueError("Base prompt is not defined. Provide a valid base_prompt or prompt_file.")
        ...
        # Validate extra_context for list matching
        if isinstance(prompt, list):
            print(f"BBB prompt: {prompt}")
            if extra_context is None or len(extra_context) != len(prompt):
                raise ValueError("extra_context must be a list with the same length as prompt.")
    
            extended_prompt = [
                base_prompt.replace("INSERT_QUERY_HERE", p)
                .replace('INSERT_TYPE_HERE', input_type)
                .replace('EXTRA_CONTEXT_HERE', str(ec))
                for p, ec in zip(prompt, extra_context)
            ]
            print(f"CCC Extended prompt: {extended_prompt}")
        elif isinstance(prompt, str):
            extended_prompt = [
                base_prompt.replace("INSERT_QUERY_HERE", prompt)
                .replace('INSERT_TYPE_HERE', input_type)
                .replace('EXTRA_CONTEXT_HERE', str(extra_context))
            ]
            print("is instance extened prompt: ")
        ...
        # Call the forward_ method to get the result
        result = self.forward_(extended_prompt)
        print(f"Result: {result}")
        if result is None:
            print("Result from forward_ is None.")
            return None
    
        # Ensure result is not empty if prompt is not a list
        if not isinstance(prompt, list):
            if len(result) == 0:
                raise ValueError("Result is empty; cannot return the first element.")
            result = result[0]
    ...
    def forward_(self, extended_prompt):
        print("OOOOOOOOOOOOOOOOOOOOOOOOO ext pr:")
        print(extended_prompt)
        if len(extended_prompt) > self.max_batch_size:
            ...
            print(f"Length is more than max_batch_size: {len(extended_prompt)}")
            print(f"Response: {response}")
            return response
        try:
            print(f"Calling codex_helper with: {extended_prompt}")  # Debugging line
            response = codex_helper(extended_prompt)
            print(f"Response from codex_helper: {response}")  # Debugging line
            ...
            response_1 = self.forward_(sub_batch_1) if len(sub_batch_1) > 0 else []
            response_2 = self.forward_(sub_batch_2) if len(sub_batch_2) > 0 else []
